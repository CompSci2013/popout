...
Dynamically positioned UI (CDK overlays, dropdowns, tooltips) may compute positions relative to the parent window's viewport, not the popout's. This is a known CDK Portal limitation. For this tile component (card + input), it's not an issue.
